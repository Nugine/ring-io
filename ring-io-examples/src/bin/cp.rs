...
use ring_io::sys::{FsyncFlags, SQEFlags};
use ring_io::{CompletionQueue, PrepareSQE, Ring, SubmissionQueue, CQE};
...
fn copy_file(
    ...
    loop {
        ...
        while current_offset < cp_size {
            ...
            let sqe_index = match sq.pop_sqe() {
                None => break,
                Some(sqe) => sqe,
            };
        ...
        // reap available CQEs
        let mut cqes_buf: [MaybeUninit<&CQE>; RING_ENTRIES as usize] = unsafe { mem::zeroed() };
        ...
        unsafe {
            sq.modify_sqe(sqe_index, |sqe| {
                sqe.prep_fsync(dst_fd, FsyncFlags::empty())
                    .enable_flags(SQEFlags::IO_DRAIN)
            });
            sq.push_sqe(sqe_index)
        }
    ...
    debug_assert_eq!(sq.available(), RING_ENTRIES);
